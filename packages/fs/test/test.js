...
(async () => {
  ...
  await fs.mkdir("/t2");
  await fs.writeFile("/t2/a.md", "aa");
  await fs.mkdir("/t2/t2-1");
  await fs.writeFile("/t2/t2-1/b.md", "bb");
  await fs.removeDir("/t2", { recursive: true });

  // await fs.removeFile("/rn_file02").catch((err) => console.warn(err));
